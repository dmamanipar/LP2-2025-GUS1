...
export class MainVentaComponent implements OnInit{
  ...
  producControl= new FormControl<string| ProductoReport>('');
  clientControl = new FormControl<string | Cliente>('');


  products: ProductoReport[]=[];
  clientes: Cliente[] = [];

  productFiltered$!: Observable<ProductoReport[]>;
  ...
  loadInitialData() {
    ...
    this.productService.findAllOT().subscribe(data => {
      this.productService.setProductosSubject(data);
    });
    this.productService.getProductosSubject().subscribe(data => {
      this.products = data;
    });
    ...
    if (this.salesFormGroup.valid) {
     let id: number= Number(sessionStorage.getItem(environment.DATA_USERLOGIN));
      ...
      const dtoForm: VentCarritosCA = new VentCarritosCA(
        ...
        (this.producControl.value as ProductoReport).idProducto,
        (this.producControl.value as ProductoReport).nombre,
